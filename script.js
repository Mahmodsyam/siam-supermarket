(function () {
  ...
  function setupBottomNav() {
    ...
    function updateActive() {
      ...
      Array.prototype.forEach.call(items, function (item) {
        var isActive = item.getAttribute('data-section') === current;
    ...
    function moveIndicator(id) {
      if (!indicator) return;
      var activeItem = nav.querySelector('.nav-item[data-section="' + id + '"]');
      if (!activeItem) { indicator.classList.remove('visible'); return; }
      var navRect = nav.querySelector('.bottom-nav-inner').getBoundingClientRect();
      var itemRect = activeItem.getBoundingClientRect();
      var left = itemRect.left - navRect.left + 6;
      var width = itemRect.width - 12;
    ...
    nav.addEventListener('click', function (e) {
      ...
      if (!targetId || targetId === '#') return removeRipple(e, item);
      var target = document.querySelector(targetId);
      if (target) {
        target.scrollIntoView({ behavior: 'smooth', block: 'start' });
        setTimeout(function () { updateActive(); }, 400);
      }
      createRipple(e, item);
    });
    ...
    function removeRipple(e, item) { /* no-op for empty links */ }

    updateActive();
    if (indicator) setTimeout(function () { moveIndicator(sections[0].id); }, 100);
  }
